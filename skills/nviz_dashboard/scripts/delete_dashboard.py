...
"""Delete an nviz dashboard configuration from Qdrant."""
...
def main():
    """Execute the main entry point to delete a dashboard."""
    parser = argparse.ArgumentParser(
        description='Delete an nviz dashboard'
    )
    parser.add_argument(
        '--name',
        required=True,
        help='Name of the dashboard to delete'
    )
    parser.add_argument(
        '--host',
    ...
    try:
        ...
        if not args.force:
            confirm = input(f"Delete dashboard '{args.name}'? [y/N]: ")
            if confirm.lower() != 'y':
                print('Aborted.')
                return 0
        ...
        print(f"Dashboard '{args.name}' deleted successfully.")
        return 0
